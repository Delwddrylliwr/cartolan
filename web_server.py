...
from main_game import InteractiveSimulation
...
new_player_sockets = {} #refernced by players
    ...
    def join_game(self):
        ...
        while True:
            if len(new_game_types) == 0:
                ...
                print("Prompting client at " +self.address+ " with: " +prompt_text)
                ...
                while not new_game_type in valid_options:
                    new_game_type = self.socket.recv()
                ...
                for player_num in range(num_client_players):
                    ...
                    new_player_sockets[player] = self
                for player_num in range(num_virtual_players):
                    ...
                    client_players[self].append(player)
                    new_player_sockets[player] = None
            else:
                ...
                num_existing_players = len(new_player_sockets[game_id])
                ...
                num_existing_players = len(new_player_sockets[game_id])
                num_spaces = len(new_game_colours[game_id]) - num_existing_players
                ...
                for player_num in range(num_existing_players, num_existing_players + num_client_players):
                    ...
                    player = PlayerHuman(player_colour)
                    client_players[self].append(player)
                    new_player_sockets[game_id][player] = self
                #If the game is now full then start it off
                if num_client_players == num_spaces:
                    random.shuffle(new_game_players)
                    self.game = InteractiveSimulation.setup_simulation(self.players
                    ...
                    games[game_id] = {"player_sockets":new_player_sockets.pop(game_id)
                                        , "player_colours":new_game_colours.pop(game_id)
                    ...
                    for client in games[game_id]["clients"]:
                        ...
                        game_vis = WebServerVisualisation(self.game, dimensions, origin, client.socket, client.width, client.height)
