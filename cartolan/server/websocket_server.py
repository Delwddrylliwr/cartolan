...
class ClientSocket(WebSocket):
    ...
    def start_game(self, game_id):
        ...
        if (self.game.winning_silks_difference is not None
                and self.game.silks_difference >= self.game.winning_silks_difference):
...
def main(argv=None):
    '''Regenerates the web client asset manifests and runs the WebSocket server.'''
    if argv is None:
        argv = sys.argv[1:]
    generate_tile_manifest()
    generate_card_manifest()
    if argv:
        print("Server port taken to be " + argv[0])
        port = argv[0]
    else:
        port = DEFAULT_PORT
